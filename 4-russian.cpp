...
// Takes matrix input
void matrix_input(vector<vector<bool>> &A, int n) {
	for (int i = 0; i < n; ++i) {
		vector<bool> row;
		for (int j = 0; j < n; ++j) {
			bool x;
...
void pad_matrices(vector<vector<bool>> &A, vector<vector<bool>> &B, int n, int pad_length) {
...
int main() {
	...
	int m = floor(log2(n)); // Size of a partition of matrix
	int k = ceil(n/(float)m); // Number of partitions
	...
	// Pad matrices
	int pad_length = m*k - n;
	pad_matrices(A, B, n, pad_length);
